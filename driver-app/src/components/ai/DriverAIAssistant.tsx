...
export const DriverAIAssistant: React.FC = () => {
  ...
  const generateAiAnswer = (query: string): { reply: string; action?: string } => {
    ...
    // Default intelligent driver assistant response
    return {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-float border border-slate-100 flex flex-col h-[82vh] max-h-[680px] overflow-hidden">
        ...
        <div className="px-5 py-3.5 border-b border-slate-100 bg-slate-900 text-white flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-400/40 flex items-center justify-center">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-bold bg-emerald-500/30 text-emerald-300 px-1.5 py-0.2 rounded border border-emerald-500/40">
                  Fleet Copilot
                </span>
              </div>
          ...
          <div className="flex items-center gap-1">
            {isSpeaking && (
              <span className="flex items-center gap-1 text-[11px] text-emerald-400 font-bold bg-emerald-500/20 px-2 py-0.5 rounded-full border border-emerald-500/30">
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-3.5 bg-slate-50">
          {messages.map((m, idx) => (
            ...
            >
              <div
                className={`max-w-[86%] p-3.5 rounded-2xl text-xs leading-relaxed shadow-2xs ${
                  m.sender === 'driver'
              ...
              >
                ...
                {m.action && (
                  <button
                    ...
                    className="mt-2.5 inline-flex items-center gap-1.5 text-[11px] font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-3 py-1 rounded-xl border border-emerald-200 transition active-press"
                  >
        ...
        <div className="p-3 bg-white border-t border-slate-100">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] text-slate-400">One-tap query</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {quickChips.map((chip, i) => (
              <button
                ...
                className="p-2 bg-slate-50 hover:bg-emerald-50 hover:text-emerald-800 hover:border-emerald-200 border border-slate-200 rounded-xl text-left text-[11px] font-bold text-slate-700 transition active-press truncate"
              >
                {chip.label}
              </button>
            ))}
        ...
        >
          <input
            ...
            className="flex-1 bg-slate-100 border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 placeholder:text-slate-400"
          />
          <button
            type="submit"
            disabled={!inputVal.trim()}
            className="w-10 h-10 rounded-xl bg-slate-900 hover:bg-emerald-600 disabled:opacity-40 text-white flex items-center justify-center transition active-press"
          >
